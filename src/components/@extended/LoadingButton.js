...
const LoadingButtonStyle = styled(MuiLoadingButton, { shouldForwardProp: (prop) => prop !== "shape" && prop !== "variant" })(({ theme, variant, shape, color, loading, loadingPosition }) => ({
	"::after": {
		content: '""',
		display: "block",
		position: "absolute",
...
const LoadingButton = forwardRef(({ variant = "text", shape, children, color = "primary", ...others }, ref) => {
	...
	return (
		<LoadingButtonStyle ref={ref} variant={variant} shape={shape} theme={theme} loadingPosition={others.loadingPosition} loading={others.loading} color={color} {...others}>
			{children}
		</LoadingButtonStyle>
	);
});
