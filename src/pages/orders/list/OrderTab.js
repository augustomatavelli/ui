		...
		<Box sx={{ width: "100%" }}>
			<Box sx={{ borderBottom: 1, borderColor: "divider" }}>
				<Tabs value={value} onChange={handleChange} TabIndicatorProps={{ style: { backgroundColor: indicatorColors[value] } }} variant="scrollable">
					<Tab label={`Todas${!loadingOrder ? ` (${totalTabOrders?.amount_total ?? 0})` : ""}`} {...a11yProps(0)} sx={style(theme, value, 0, "secondary")} disabled={loadingOrder} />
					<Tab label={`Abertas${!loadingOrder ? ` (${totalTabOrders?.amount_open ?? 0})` : ""}`} {...a11yProps(1)} sx={style(theme, value, 1, "primary")} disabled={loadingOrder} />
					<Tab label={`Em execução${!loadingOrder ? ` (${totalTabOrders?.amount_execution ?? 0})` : ""}`} {...a11yProps(2)} sx={style(theme, value, 2, "warning")} disabled={loadingOrder} />
					<Tab label={`Finalizadas${!loadingOrder ? ` (${totalTabOrders?.amount_finalized ?? 0})` : ""}`} {...a11yProps(3)} sx={style(theme, value, 3, "success")} disabled={loadingOrder} />
					<Tab label={`Canceladas${!loadingOrder ? ` (${totalTabOrders?.amount_canceled ?? 0})` : ""}`} {...a11yProps(4)} sx={style(theme, value, 4, "error")} disabled={loadingOrder} />
				</Tabs>
			</Box>
			{[0, 1, 2, 3, 4].map((index) => (
